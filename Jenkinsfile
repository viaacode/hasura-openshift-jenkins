                ...
                script {
                    openshift.withCluster() {
                        openshift.withProject("shared-components") {
                             ...
                             oc -n shared-components env dc/sc-events-hasura-qas HASURA_GRAPHQL_DATABASE_URL=postgres://${POSTGRESQL_USER}:${POSTGRESQL_PASSWORD}@sc-events-db-qas:5432/${DB_NAME}
                               '''
                                            }
                                        }
                } // script
